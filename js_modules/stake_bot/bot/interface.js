...
if (variant === 'lng') {
        ...
        buttons = [[lng[lang].add_account, lng[lang].accounts, lng[lang].change_tags, lng[lang].keywords], [lng[lang].exclude_authors, lng[lang].help, lng[lang].lang]];
    } else if (variant === 'on_off') {
        buttons = [[lng[lang].on, lng[lang].off, lng[lang].back, lng[lang].home]];
    } else if (variant.indexOf('@') > -1 && variant.indexOf('accounts_buttons') === -1 && variant.indexOf('unvote') === -1 && variant.indexOf('upvoteButton') === -1) {
        ...
        buttons = [[lng[lang].min_energy, lng[lang].curators, lng[lang].favorits], [lng[lang].curators_mode, lng[lang].curators_coeff, lng[lang].favorits_percent], [lng[lang].auto_donate, lng[lang].back, lng[lang].home]];
    } else if (variant.indexOf('unvote@') > -1) {
        let post = variant.split('@')[1];
        buttons = [[[lng[lang].unvote + post, lng[lang].unvote_button], [`${lng[lang].postActions_button}|${post.split('_')[1]}`, lng[lang].postActions_button]]];
    } else if (variant.indexOf('accounts_buttons') > -1) {
        buttons = JSON.parse(variant.split('accounts_buttons')[1]);
    }     else if (variant.indexOf('upvoteButton_') > -1) {
        let post = variant.split('_')[1];
        buttons = [[[`${lng[lang].postActions_button}|${post}`, lng[lang].postActions_button], [lng[lang].rate_button, lng[lang].rate_button]]];
    ...
    buttons = [[['-100', '-100'], ['-75', '-75'], ['-50', '-50'], ['-25', '-25'], ['-10', '-10']], [['0', '0'], ['10', '10'], ['25', '25']], [['50', '50'], ['75', '75'], ['100', '100']], [[lng[lang].skip, lng[lang].skip], [lng[lang].cancel, lng[lang].cancel]]];
}     else if (variant === 'cancel_skip') {
    buttons = [[lng[lang].from_donate_pool, lng[lang].skip], [lng[lang].cancel]];
...
if (referer.referers.length > 0) {
    ...
    if (extra_referer.referers.length > 0) {
        ...
        if (lng[user.lng] && message !== lng[user.lng].back) {            
            if (typeof message !== 'undefined' && message.indexOf('r') > -1) {
                let ref_id = parseInt(message.split(' r')[1]);
                let referer = await udb.getUserByRefererCode(ref_id);
        ...
        if (referer.referers.length > 0) {
            ...
            refs.push(referer.referers[0])
                ...
                await udb.updateUser(id, refs, user.lng, user.status, message, user.referer_code, user.tags, user.keywords, user.exclude_authors);
            } else {
                let extra_referer = await udb.getUser(ref_id);
                if (extra_referer) {
                    let text = lng[extra_referer.lng].new_referal1 + `https://t.me/golos_stake_bot?start=r${extra_referer.referer_code}`;
        ...
        await udb.updateUser(id, refs, user.lng, user.status, message, user.referer_code, user.tags, user.keywords, user.exclude_authors);
                    ...
                    await udb.updateUser(id, user.referers, user.lng, user.status, message, user.referer_code, user.tags, user.keywords, user.exclude_authors);
                }
            }
            } else {
                await udb.updateUser(id, user.referers, user.lng, user.status, message, user.referer_code, user.tags, user.keywords, user.exclude_authors);
            ...
            await udb.updateUser(id, user.referers, user.lng, user.prev_status, user.status, user.referer_code, user.tags, user.keywords, user.exclude_authors);
        }        
    }
...
await botjs.sendMSG(id, text, btns, true);
                                                            ...
                                                            await botjs.sendMSG(id, text, btns, false);
                                                                            ...
                                                                            await udb.updateUser(id, user.referers, user.lng, user.status, 'delete_' + login, user.referer_code, user.tags, user.keywords, user.exclude_authors);
                                                    ...
                                                    let btns = await keybord(user.lng, 'cancel');
                                                    await botjs.sendMSG(id, text, btns, false);
                                                } else if (message.indexOf('@') > -1 && user.status.indexOf(lng[user.lng].news) === -1 && message.indexOf(lng[user.lng].rate_button) === -1 && message.indexOf(lng[user.lng].postActions_button) === -1) {
                                                                            ...
                                                                            if (acc && acc.id === id) {
                                                                                ...
                                                                                let btns = await keybord(user.lng, message);
                                                                                        ...
                                                                                        posting_public_keys.push(key[0]);
                                                                                            ...
                                                                                            btns = await keybord(user.lng, 'cancel');
                                                                                            await udb.updateUser(id, user.referers, user.lng, user.status, 'changed_posting_' + login + '_' + JSON.stringify(posting_public_keys), user.referer_code, user.tags, user.keywords, user.exclude_authors);
                                                                                        } else {
                                                                                            await udb.updateUser(id, user.referers, user.lng, user.status, 'change_account', user.referer_code, user.tags, user.keywords, user.exclude_authors);
...
${lng[user.lng].audo_donate_description}:
<code>${acc.auto_donate}</code>

${lng[user.lng].curators_mode}: ${acc.curators_mode},
${lng[user.lng].curators_coeff}: ${acc.curators_coeff},
${lng[user.lng].favorits_percent}: ${acc.favorits_percent}.`;
                                                                    let btns = await keybord(user.lng, 'auto_curator');
                                                                await botjs.sendMSG(id, text, btns, false);
                                                                await udb.updateUser(id, user.referers, user.lng, user.status, 'auto_curator@' + login, user.referer_code, user.tags, user.keywords, user.exclude_authors);
    }
                                                                ...
                                                                let login = user.status.split('@')[1];
                                                                    ...
                                                                    let btns = await keybord(user.lng, 'cancel');
                                                                await botjs.sendMSG(id, text, btns, false);
                                                                await udb.updateUser(id, user.referers, user.lng, user.status, 'minEnergy_' + login, user.referer_code, user.tags, user.keywords, user.exclude_authors);
    ...
    console.error(er);
                                                                ...
                                                                let login = user.status.split('@')[1];
                                                                    ...
                                                                    let btns = await keybord(user.lng, 'cancel');
                                                                await botjs.sendMSG(id, text, btns, false);
                                                                await udb.updateUser(id, user.referers, user.lng, user.status, 'curators|' + login, user.referer_code, user.tags, user.keywords, user.exclude_authors);
                                                                ...
                                                                let login = user.status.split('@')[1];
                                                                    ...
                                                                    let btns = await keybord(user.lng, 'on_off');
                                                                await botjs.sendMSG(id, text, btns, false);
                                                                await udb.updateUser(id, user.referers, user.lng, user.status, 'curatorsMode_' + login, user.referer_code, user.tags, user.keywords, user.exclude_authors);
                                                                ...
                                                                await botjs.sendMSG(id, text, btns, false);
                                                                await udb.updateUser(id, user.referers, user.lng, user.status, 'favorits|' + login, user.referer_code, user.tags, user.keywords, user.exclude_authors);
                                                            } else if (user && user.lng && message === lng[user.lng].auto_donate && user.status.indexOf('auto_curator@') > -1) {
                                                                let login = user.status.split('@')[1];
                                                                    let text = lng[user.lng].auto_donate_text;
                                                                    let btns = await keybord(user.lng, 'cancel');
                                                                await botjs.sendMSG(id, text, btns, false);
                                                                await udb.updateUser(id, user.referers, user.lng, user.status, 'autoDonate_' + login, user.referer_code, user.tags, user.keywords, user.exclude_authors);
                                                                ...
                                                                let login = user.status.split('@')[1];
                                                                    ...
                                                                    let btns = await keybord(user.lng, 'cancel');
                                                                await botjs.sendMSG(id, text, btns, false);
                                                                await udb.updateUser(id, user.referers, user.lng, user.status, 'favoritsPercent_' + login, user.referer_code, user.tags, user.keywords, user.exclude_authors);
                                                            } else if (user && user.lng && message === lng[user.lng].curators_coeff && user.status.indexOf('auto_curator@') > -1) {
                                                                let login = user.status.split('@')[1];
                                                                    let text = lng[user.lng].curators_coeff_text;
                                                                    let btns = await keybord(user.lng, 'cancel');
                                                                await botjs.sendMSG(id, text, btns, false);
                                                                await udb.updateUser(id, user.referers, user.lng, user.status, 'curatorsCoeff_' + login, user.referer_code, user.tags, user.keywords, user.exclude_authors);

                                                            } else if (user && user.lng && message === lng[user.lng].exclude_authors) {
                                                                    let text = `${lng[user.lng].exclude_authors_text}: <code>${user.exclude_authors}</code>`;
                                                                    let btns = await keybord(user.lng, 'cancel');
                                                                await botjs.sendMSG(id, text, btns, false);
                                                                await udb.updateUser(id, user.referers, user.lng, user.status, 'excludeAuthors', user.referer_code, user.tags, user.keywords, user.exclude_authors);
                                                            } else if (user && user.lng && message === lng[user.lng].keywords) {
                                                                    let text = `${lng[user.lng].keywords_text}: <code>${user.keywords}</code>`;
                                                                    let btns = await keybord(user.lng, 'cancel');
                                                                await botjs.sendMSG(id, text, btns, false);
                                                                await udb.updateUser(id, user.referers, user.lng, user.status, 'keywords', user.referer_code, user.tags, user.keywords, user.exclude_authors);
                                                                ...
                                                                if (get_account && get_account.length > 0) {
                                                                    ...
                                                                    btns = await keybord(user.lng, 'cancel');
                                                                    await udb.updateUser(id, user.referers, user.lng, user.status, 'typed_rate@' + account.login + ':' + parseFloat(acc.tip_balance), user.referer_code, user.tags, user.keywords, user.exclude_authors);
                                                                } else {
                                                                    await udb.updateUser(id, user.referers, user.lng, user.status, 'send_rate', user.referer_code, user.tags, user.keywords, user.exclude_authors);
                                                                    text = lng[user.lng].not_account;
                                                                    btns = await keybord(user.lng, 'home');
                                                                }
                                                            } else {
                                                                text = lng[user.lng].account_not_add + ` ${message}`;
                                                                btns = await keybord(user.lng, 'home');
                                                            }
                                                            await botjs.sendMSG(id, text, btns, false);
                                                            } else if (user && user.lng && message === lng[user.lng].rate_button && user.status.indexOf('@') > -1) {
                                                                ...
                                                                if (get_account && get_account.length > 0) {
                                                                    text = lng[user.lng].type_rate + acc.tip_balance;
                                                                    btns = await keybord(user.lng, 'cancel');
                                                                    await udb.updateUser(id, user.referers, user.lng, user.status, 'typed_rate@' + login + ':' + parseFloat(acc.tip_balance), user.referer_code, user.tags, user.keywords, user.exclude_authors);
                                                                } else {
                                                                    await udb.updateUser(id, user.referers, user.lng, user.status, 'send_rate', user.referer_code, user.tags, user.keywords, user.exclude_authors);
                                                                    text = lng[user.lng].not_account;
                                                                    btns = await keybord(user.lng, 'home');
...
await botjs.sendMSG(364096327, text, btns, false);                                                                
...
} else if (user && user.lng && message.indexOf(lng[user.lng].postActions_button) > -1 || user && user.lng && message.indexOf(lng[user.lng].donate_button) > -1) {
    let postId = message.split('|')[1];
    let text = lng[user.lng].postActions_select_account;
    ...
    buttons[key].push([`actionsPost ${acc.login} ${postId}`, `@${acc.login}`]);
    if (n % 2 == 0) {
    key++;
    }
    n++;
...
await botjs.sendMSG(id, text, btns, true);
} else if (user && user.lng && message.indexOf('actionsPost ') > -1) {
    let login = message.split(' ')[1];
    let account = await adb.getAccount(login);
    if (account && account.id !== id) return;
    let postId = message.split(' ')[2];
    let get_account = await methods.getAccount(login);
    if (get_account && get_account.length > 0) {
        let acc = get_account[0];
        ...
        let award_data = {};
        award_data.acc = account;
        award_data.account = acc;
        award_data.props = props;
        award_data.postId = postId;
            text = `${lng[user.lng].type_vote} ${account.login}. ${lng[user.lng].type_vote_percent}: ${charge}%`;
                btns = await keybord(user.lng, 'send_vote');
                await udb.updateUser(id, user.referers, user.lng, user.status, 'voteing_' + JSON.stringify(award_data), user.referer_code, user.tags, user.keywords, user.exclude_authors);                
            } else {
        ...
        let btns = await keybord(message, '');
                    await udb.updateUser(id, user.referers, message, user.status, message, user.referer_code, user.tags, user.keywords, user.exclude_authors);
                    await botjs.sendMSG(id, text, btns, false);
...
posting_public_keys.push(key[0]);
    ...
    text = lng[user.lng].type_posting;
    btns = await keybord(user.lng, 'cancel');
    await udb.updateUser(id, user.referers, user.lng, user.status, 'login_' + message + '_' + JSON.stringify(posting_public_keys), user.referer_code, user.tags, user.keywords, user.exclude_authors);
} else {
    await udb.updateUser(id, user.referers, user.lng, user.status, 'add_account', user.referer_code, user.tags, user.keywords, user.exclude_authors);
    text = lng[user.lng].not_account;
    btns = await keybord(user.lng, 'home');
}
await botjs.sendMSG(id, text, btns, false);
...
await adb.updateAccount(id, user.referer_code, login, sjcl.encrypt(login + '_postingKey_stakebot', message), false, 100, '', '', 'replay', 1, 0, '10 1');
await udb.updateUser(id, user.referers, user.lng, user.status, 'saved_data', user.referer_code, user.tags, user.keywords, user.exclude_authors);
text = lng[user.lng].saved_true;
btns = await keybord(user.lng, 'home');
await botjs.sendMSG(id, text, btns, false);
} else {
    await udb.updateUser(id, user.referers, user.lng, user.status, lng[user.lng].home, user.referer_code, user.tags, user.keywords, user.exclude_authors);
...
} else if (user.status&& typeof user.status !== 'undefined' && user.lng && lng[user.lng] && user.status.indexOf('changed_posting_') > -1) {
    let [command, login, posting_public_keys] = user.status.split('_');
    let text = '';
let btns;
try {
    const public_wif = await methods.wifToPublic(message);
    ...
    let acc = await adb.getAccount(login);
    if (!acc) {
        acc.to_vesting_shares = false;
        acc.min_energy = 100;
        acc.curators = '';
        acc.favorits = '';
        acc.curators_mode = 'replay';
        acc.curators_coeff = 1;
        acc.favorits_percent = 0;
        acc.auto_donate = '10 1';
    }
        await adb.updateAccount(id, user.referer_code, login, sjcl.encrypt(login + '_postingKey_stakebot', message), acc.to_vesting_shares, acc.min_energy, acc.curators, acc.favorits, acc.curators_mode, acc.curators_coeff, acc.favorits_percent, acc.auto_donate);
                            await udb.updateUser(id, user.referers, user.lng, user.status, 'added_posting_key', user.referer_code, user.tags, user.keywords, user.exclude_authors);
...
if (energy && energy > 0) {
    await adb.updateAccount(id, user.referer_code, login, acc.posting_key, acc.to_vesting_shares, energy, acc.curators, acc.favorits, acc.curators_mode, acc.curators_coeff, acc.favorits_percent, acc.auto_donate);
...
let btns = await keybord(user.lng, 'home');
await botjs.sendMSG(id, text, btns, false);
} else if (user.lng && lng[user.lng] && user.status.indexOf('curators|') > -1) {
    let login = user.status.split('|')[1];
    let acc = await adb.getAccount(login);
    let text = '';
    if (acc && acc.id === id) {
...
if (accs && accs.length === curators.length) {
    await adb.updateAccount(id, user.referer_code, login, acc.posting_key, acc.to_vesting_shares, acc.min_energy, message, acc.favorits, acc.curators_mode, acc.curators_coeff, acc.favorits_percent, acc.auto_donate);
...
await adb.updateAccount(id, user.referer_code, login, acc.posting_key, acc.to_vesting_shares, acc.min_energy, acc.curators, acc.favorits, mode, acc.curators_coeff, acc.favorits_percent, acc.auto_donate);
}                        
    await udb.updateUser(id, user.referers, user.lng, user.status, 'auto_curator@' + login, user.referer_code, user.tags, user.keywords, user.exclude_authors);
    let btns = await keybord(user.lng, 'home');
    await botjs.sendMSG(id, text, btns, false);
} else if (user.lng && lng[user.lng] && user.status.indexOf('favorits|') > -1) {
    let login = user.status.split('|')[1];
    let acc = await adb.getAccount(login);
    let text = '';
    if (acc && acc.id === id && message === lng[user.lng].import_subs) {
        ...
        if (following !== 'error') {
            await adb.updateAccount(id, user.referer_code, login, acc.posting_key, acc.to_vesting_shares, acc.min_energy, acc.curators, following.join(), acc.curators_mode, acc.curators_coeff, acc.favorits_percent, acc.auto_donate);
...
if (accs && accs.length === favorits.length) {
    await adb.updateAccount(id, user.referer_code, login, acc.posting_key, acc.to_vesting_shares, acc.min_energy, acc.curators, message, acc.curators_mode, acc.curators_coeff, acc.favorits_percent, acc.auto_donate);
...
await udb.updateUser(id, user.referers, user.lng, lng[user.lng].home, '@' + login, user.referer_code, user.tags, user.keywords, user.exclude_authors);
let btns = await keybord(user.lng, '@' + login);
await botjs.sendMSG(id, text, btns, false);
} else if (user.lng && lng[user.lng] && user.status.indexOf('autoDonate_') > -1) {
    let login = user.status.split('_')[1];
    let acc = await adb.getAccount(login);
    let text = '';
    if (acc && acc.id !== id) return;
    let [percent, coeff] = message.split(' ');
    if (typeof coeff === 'undefined' || isNaN(coeff)) coeff = 1;
if (percent && !isNaN(percent) && percent >= 1) {
    await adb.updateAccount(id, user.referer_code, login, acc.posting_key, acc.to_vesting_shares, acc.min_energy, acc.curators, acc.favorits, acc.curators_mode, acc.curators_coeff, acc.favorits_percent, `${percent} ${coeff}`);
    text = lng[user.lng].auto_donate_saved;
} else {
    text = lng[user.lng].auto_donate_not_valid;
}
let btns = await keybord(user.lng, 'home');
await botjs.sendMSG(id, text, btns, false);
...
if (percent && percent > 0) {
    await adb.updateAccount(id, user.referer_code, login, acc.posting_key, acc.to_vesting_shares, acc.min_energy, acc.curators, acc.favorits, acc.curators_mode, acc.curators_coeff, percent, acc.auto_donate);
...
let btns = await keybord(user.lng, 'home');
await botjs.sendMSG(id, text, btns, false);
} else if (user.lng && lng[user.lng] && user.status.indexOf('curatorsCoeff_') > -1) {
    let login = user.status.split('_')[1];
    let acc = await adb.getAccount(login);
    let text = '';
    if (acc && acc.id === id) {
    let coeff = parseFloat(message);
if (coeff && coeff > 0) {
    await adb.updateAccount(id, user.referer_code, login, acc.posting_key, acc.to_vesting_shares, acc.min_energy, acc.curators, acc.favorits, acc.curators_mode, coeff, acc.favorits_percent, acc.auto_donate);
    text = lng[user.lng].curators_coeff_saved;
} else {
    text = lng[user.lng].curators_coeff_not_valid;
}
} else {
    text = lng[user.lng].account_not_add;
}
let btns = await keybord(user.lng, 'home');
await botjs.sendMSG(id, text, btns, false);
} else if (user.lng && lng[user.lng] && user.status.indexOf('excludeAuthors') > -1) {
    let text = '';
let authors = message.split(',');
let accs = await methods.getAccounts(authors);
if (accs && accs.length === authors.length) {
    await udb.updateUser(id, user.referers, user.lng, user.status, lng[user.lng].home, user.referer_code, user.tags, user.keywords, message);
...
let btns = await keybord(user.lng, 'home');
await botjs.sendMSG(id, text, btns, false);
await udb.updateUser(id, user.referers, user.lng, user.status, lng[user.lng].home, user.referer_code, user.tags, message, user.exclude_authors);
...
if (amount <= max && amount >= 0.1) {
    await udb.updateUser(id, user.referers, user.lng, user.status, 'rate_' + login + ':' + amount, user.referer_code, user.tags, user.keywords, user.exclude_authors);
...
} else if (user.lng && lng[user.lng] && user.status.indexOf('typedActions~') > -1) {
    let json = user.status.split('~')[1];
    let data = JSON.parse(json);
let {postId, acc, account, props, voted} = data;
let login = acc.login;
let posting_key = acc.posting_key;
let text = '';
let post = await pdb.getPost(parseInt(postId));
if (!post || Object.keys(post).length === 0) return;
    let {author, permlink} = post;
    let wif = sjcl.decrypt(login + '_postingKey_stakebot', posting_key);
if (voted !== lng[user.lng].skip) {
    try {
        await methods.vote(wif, login, author, permlink, parseFloat(voted));
...
if (message !== lng[user.lng].skip) {
    let amount = 0;
if (voted !== lng[user.lng].skip && message === lng[user.lng].from_donate_pool) {
    const donate_amount = helpers.calcDonateFromEmission(acc, account, props, parseInt((parseFloat(voted) * 100)));
    amount = donate_amount;
} else if (!isNaN(message)) {
    amount = parseFloat(message);
}
if (amount <= parseFloat(account.tip_balance) && amount >= 0.1) {
    try {
    let full_amount = amount;
    amount *= 0.998;
    await methods.donate(wif, login, author, amount.toFixed(3) + ' GOLOS', 'https://t.me/golos_stake_bot', author, permlink);
    let fee = full_amount - amount;
    if (fee > 0.001) await methods.donate(wif, login, conf.stakebot.golos_login, fee.toFixed(3) + ' GOLOS', 'Комиссия.', author, permlink);            
...
try {
    ...
    await udb.updateUser(id, user.referers, user.lng, user.status, 'sended_rate', user.referer_code, user.tags, user.keywords, user.exclude_authors);
    let btns = await keybord(user.lng, 'home');
    await botjs.sendMSG(id, text, btns, false);
    await helpers.sleep(3000);
    await main(id, my_name, lng[user.lng].home, status);
...
console.log('Результат: ' + JSON.stringify(res));
    ...
    await udb.updateUser(id, user.referers, user.lng, user.status, 'delet_account', user.referer_code, user.tags, user.keywords, user.exclude_authors);
    let btns = await keybord(user.lng, 'home');
    await botjs.sendMSG(id, text, btns, false);
    await helpers.sleep(3000);
    await main(id, my_name, lng[user.lng].home, status);
    ...
    let login = acc.login;
    if (acc && acc.id !== id || account && account.length === 0) return;
text = lng[user.lng].type_donate + account.tip_balance;
btns = await keybord(user.lng, 'cancel_skip');
data.voted = message;
await udb.updateUser(id, user.referers, user.lng, user.status, `typedActions~${JSON.stringify(data)}`, user.referer_code, user.tags, user.keywords, user.exclude_authors);
await botjs.sendMSG(id, text, btns, false);
...
await udb.updateUser(id, user.referers, user.lng, user.status, '/start', user.referer_code, result, user.keywords, user.exclude_authors);
...
${lng[user.lng].charge}. ${members[id].charge}%`;
...
for (let tag of tags) {
    ...
    for (let user of users) {
        ...
        if (user.exclude_authors && user.exclude_authors.indexOf(opbody.author) > -1) continue;
...
${lng[user.lng].tags}:${tags_list}`;
    let btns = await keybord(user.lng, `upvoteButton_${content.id}`);
