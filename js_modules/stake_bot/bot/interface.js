...
if (variant === 'lng') {
        ...
        buttons = [[lng[lang].min_energy, lng[lang].curators, lng[lang].favorits, lng[lang].curators_mode], [lng[lang].favorits_percent, lng[lang].back, lng[lang].home]];
        ...
        let post = variant.split('@')[1];
        buttons = [[[`${lng[lang].vote} ${post}`, lng[lang].vote], [`${lng[lang].donate_button} ${post}`, lng[lang].donate_button], [lng[lang].rate_button, lng[lang].rate_button]]];
...
await botjs.sendMSG(id, text, btns, true);
    ...
    if (get_account && get_account.length > 0) {
        ...
        btns = await keybord(user.lng, 'cancel');
        await udb.updateUser(id, user.referers, user.lng, user.status, 'typedDonate_' + post_id + ':' + login + ':' + parseFloat(acc.tip_balance), user.referer_code, user.tags, user.keywords, user.exclude_authors);
    } else {
        text = lng[user.lng].not_connection;
        btns = await keybord(user.lng, 'home');
    }
...
try {
    ...
    if (!acc) {
        ...
        await adb.updateAccount(id, user.referer_code, login, sjcl.encrypt(login + '_postingKey_stakebot', message), acc.to_vesting_shares, acc.min_energy, acc.curators, acc.favorits, acc.curators_mode, acc.favorits_percent);
...
if (energy && energy > 0) {
    await adb.updateAccount(id, user.referer_code, login, acc.posting_key, acc.to_vesting_shares, energy, acc.curators, acc.favorits, acc.curators_mode, acc.favorits_percent);
...
if (accs && accs.length === curators.length) {
    await adb.updateAccount(id, user.referer_code, login, acc.posting_key, acc.to_vesting_shares, acc.min_energy, message, acc.favorits, acc.curators_mode, acc.favorits_percent);
...
await adb.updateAccount(id, user.referer_code, login, acc.posting_key, acc.to_vesting_shares, acc.min_energy, acc.curators, acc.favorits, mode, acc.favorits_percent);
}                        
    await udb.updateUser(id, user.referers, user.lng, user.status, 'auto_curator@' + login, user.referer_code, user.tags, user.keywords, user.exclude_authors);
    let btns = await keybord(user.lng, 'home');
    await botjs.sendMSG(id, text, btns, false);
    ...
    if (acc && acc.id === id && message === lng[user.lng].import_subs) {
        ...
        if (following !== 'error') {
            await adb.updateAccount(id, user.referer_code, login, acc.posting_key, acc.to_vesting_shares, acc.min_energy, acc.curators, following.join(), acc.curators_mode, acc.favorits_percent);
...
if (accs && accs.length === favorits.length) {
    await adb.updateAccount(id, user.referer_code, login, acc.posting_key, acc.to_vesting_shares, acc.min_energy, acc.curators, message, acc.curators_mode, acc.favorits_percent);
...
if (percent && percent > 0) {
    await adb.updateAccount(id, user.referer_code, login, acc.posting_key, acc.to_vesting_shares, acc.min_energy, acc.curators, acc.favorits, acc.curators_mode, percent);
...
let btns;
if (amount <= max && amount >= 0.1) {
await udb.updateUser(id, user.referers, user.lng, user.status, 'donate_' + login + ':' + amount + ':' + post_id, user.referer_code, user.tags, user.keywords, user.exclude_authors);
...
console.log('Результат: ' + JSON.stringify(res));
    ...
    await udb.updateUser(id, user.referers, user.lng, user.status, '/start', user.referer_code, user.tags, user.keywords, user.exclude_authors);
    } catch(e) {
